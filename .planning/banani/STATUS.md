...
### Phase 18 — Kanban board + Admin dashboard (2026-08-07)
- [x] `kanban-theses` — "Kanban — Étapes de thèse" (`KanbanTheses.jsx`) — `frontend/src/app/students/kanban/page.tsx`, ENCADRANT-only. Zero new backend — `GET /api/theses?limit=50` already returns everything a card needs (`stage`, `topic`, `deadlines`, `_count.comments`); the 5 columns map 1:1 onto `Thesis.stage`'s 5 real values. Entry points: new "Kanban" button on `/dashboard` (next to "Voir tout"/"Ajouter un étudiant") and on `/students` (next to "Ajouter un étudiant") — both plain `Link`s, no new nav item (Sidebar's "Mes étudiants" stays active via the existing prefix match).
- [x] Shared: `KanbanCard` under `frontend/src/components/dashboard/`; reuses `AddStudentForm`, `Avatar`, `Icon`, `DashboardHeader`'s `actions` slot (previously unused).
- [x] `admin-dashboard` — "Admin — Tableau de bord" (`new_screen9.jsx`) — `frontend/src/app/admin/page.tsx`, new top-level route gated on the pre-existing `User.role` field (ADMIN/SUPERADMIN via `GET /api/admin/me`), unrelated to `profileType`.
- [x] **Scope decision (user-confirmed, see `.planning/banani/admin-dashboard.md`)**: the Banani mock shows data with no backing today — MRR, per-institution plan/country/status, "comptes inactifs" (no `lastLoginAt` anywhere), and system alerts (no alerting mechanism exists). Built as an honest MVP ("Option A"): only the 3 KPIs with real data (Encadrants inscrits, Étudiants suivis, Thèses en cours), a real Universités table (name + real encadrant/student counts, no plan/country/status columns), a genuine empty state for "Alertes système" (not fabricated alerts), and real "Nouveaux inscrits". 7 of the sidebar's 8 nav items render visibly but inert (`title="Bientôt disponible"`) — only this one screen was designed in Banani.
- [x] Backend: **new** `GET /api/admin/institutions` (institution name + real encadrant/student counts via `groupBy`, sequential not `Promise.all` — Neon `connection_limit=1`), **new** `GET /api/admin/stats` (3 real counts). Extended `GET /api/admin/users`'s `USER_SELECT` with `profileType` + `institution.name` (additive, existing tests use `toMatchObject` — unaffected) to power "Nouveaux inscrits" without a new route.
- [x] Shared: `AdminSidebar`, `AdminShell` (mobile drawer, mirrors `DashboardShell`), `AdminKpiCard`, `InstitutionsTable`, `SystemAlerts`, `RecentSignups` under `frontend/src/components/admin/` (new directory).
- [x] `Icon.tsx`: added `building-2`, `user-check`, `shield`.
- [x] format/lint/typecheck/test(752/752)/build all green.
- [x] **Full real browser QA** (Playwright + system Chrome), re-run after the Neon outage cleared: `/dashboard` and `/students` Kanban buttons confirmed earlier at 0/3 horizontal-overflow. `/students/kanban` and `/admin` re-verified end-to-end via real login (seeded ENCADRANT + SUPERADMIN QA accounts, deleted after) at 375/768/1280px — 0 horizontal overflow on both at every breakpoint. Kanban: all 5 columns render (En attente/Rédaction/Révision/Soutenance/Bloqué), one real thesis card per stage, mobile shows the intended horizontal-scroll layout (not stacked). Admin: real KPIs (4 encadrants/15 étudiants/16 thèses), Universités table populated with real per-institution counts, "Nouveaux inscrits" showing real users with role badges, "Alertes système" showing the genuine empty state — no fabricated data anywhere. Static verification (format/lint/typecheck/752 tests/build) is fully green.
- Plans: `.planning/banani/kanban-theses.md`, `.planning/banani/admin-dashboard.md`
