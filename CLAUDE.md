...
**Cron strategy.** No `setInterval` loops — Next.js / Vercel doesn't keep long-lived processes. Background work runs as **Vercel Cron** routes under `app/api/cron/<name>/route.ts`, each gated by `Authorization: Bearer ${CRON_SECRET}`. Targets: `outbox-drain` (1m), `email-queue-drain` (1m), `verification-cleanup` (hourly), `order-expiration` (5m), `webhook-log-purge` (daily), `email-job-purge` (daily — purges SENT EmailJob rows older than `EMAIL_JOB_RETENTION_DAYS`, default 30), `deadline-reminder` (daily 8h UTC — ThèseFacile: notifies each deadline's encadrant, in-app and/or email per their `DEADLINE_REMINDER` notification preference, for `Deadline.dueAt` falling within the next 3 days; `dedupeKey: deadline-reminder:${deadlineId}` makes re-runs a no-op), `scheduled-deposits` (5m — ThèseFacile: releases `Document` rows whose `scheduledAt` ("Programmer le dépôt") has passed, clearing the field and firing the same `DOCUMENT_SUBMITTED` notification the immediate-deposit path sends; until release the document is invisible to the encadrant but visible to the uploading student), `monitoring-check` (2m — Admin → Monitoring: runs the 8-service health sweep in [frontend/src/lib/server/monitoring/run-checks.ts](frontend/src/lib/server/monitoring/run-checks.ts) and applies the warning/critical incident state machine; see that section below). Multi-instance coordination still uses [frontend/src/lib/server/leader-lease.ts](frontend/src/lib/server/leader-lease.ts) Redis leases where two crons could collide. The Bictorys charge `CircuitBreaker` is still in-memory single-instance — replace with a Redis-backed variant for multi-pod prod (documented limitation).
...
**Admin monitoring center (Admin → Monitoring)** — [frontend/src/lib/server/monitoring/](frontend/src/lib/server/monitoring/). Real, server-side health checks for the 8 critical services (Application, Neon, Vercel, GitHub, Cloudinary, Resend, Upstash, Bictorys) — never fabricated: a service whose optional credentials aren't configured (`GITHUB_TOKEN`/`GITHUB_REPO`, `VERCEL_TOKEN`/`VERCEL_PROJECT_ID`) reports `UNVERIFIED` ("Non vérifiable automatiquement"), not a fake status. `run-checks.ts` is the threshold state machine: a single failure only recolors the card `WARNING` (`MONITORING_WARNING_THRESHOLD`, default 1) — a `MonitoringIncident` row is only opened once `MONITORING_CRITICAL_THRESHOLD` (default 3) consecutive failures hit, and resolved (with a computed duration) on the next success. Driven by the `monitoring-check` cron (2m, leader-lease guarded) and a SUPERADMIN-only manual `POST /api/admin/monitoring/check-now` trigger. `GET /api/admin/monitoring/status` + `/incidents` feed the `/admin/monitoring` page. All 8 checkers live under `checks/*.ts` — extend that directory (not the cron route) to add a 9th service.
...
- [frontend/src/lib/server/monitoring/checks/](frontend/src/lib/server/monitoring/checks/) — add a checker for a 9th service (one `async () => CheckResult` per file, `unverified: true` when its credentials are absent — never fabricate a status); register it in `run-checks.ts`'s `CHECKERS` array and `SERVICE_KEYS` (both `lib/server/monitoring/types.ts` and the client-safe `lib/monitoring.ts` copy)
...
- Monitoring checkers (`lib/server/monitoring/checks/*.ts`) MUST return `unverified: true` when a service's credentials are absent — never fabricate an OPERATIONAL/WARNING/CRITICAL status for something that wasn't actually checked. A single failed check MUST NOT open a `MonitoringIncident` — only `MONITORING_CRITICAL_THRESHOLD` consecutive failures does (anti-flapping).
